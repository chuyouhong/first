...
public class GPSUtils {
	...
	public static void main(String[] args) {
		...
		try {
			Object[] objs = gps.getCoordinateAddress("上海上海市北蔡镇虹南居委会浦东新区杨思");
		...
		try {
			...
			if (httpsConn != null) {
				...
				if (result != null && !result.equals("") && result.contains("<status>0</status>")) {
					String location = result.substring(result.indexOf("<location>") + 11,
					...
					// lat = lat.substring(0,
					// lat.indexOf(".")+6>lat.length()?lat.length():lat.indexOf(".")+6);
					// lng = lng.substring(0,
					// lng.indexOf(".")+6>lng.length()?lng.length():lng.indexOf(".")+6);
				}
			}
		} catch (IOException e) {
			e.printStackTrace();
			...
			if (httpsConn1 != null) {
				...
				if (result1 != null && !result1.equals("") && result1.contains("<status>0</status>")) {
					...
					if (result1.contains("<district>"))
						district = result1.substring(result1.indexOf("<district>") + 10,
								result1.indexOf("</district>"));
