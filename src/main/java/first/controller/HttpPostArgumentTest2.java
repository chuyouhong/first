		...
		if (f.isDirectory()) {
			...
			for (int j = 0; j < fList.length; j++) {
				...
				if (file.isFile()) {//http://media.thedoc.cn/upload/img/
					Map<String, String> map = httpPostArgumentTest2.SubmitPost("http://media.thedoc.cn/upload/img/?type=身份证&suffix=jpg&file=", file.getName(), path);
					//String s="update temp_doc set id_card_url='"+map.get("url")+"' where phone="+map.get("id")+";";
					...
					dataList.add(s);
					System.err.println(map.get("url")+"   ----"+map.get("id"));
				}
			}
		}
